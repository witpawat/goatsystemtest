                    ...
                    <table class="table">
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <input type="text" name="attitude[]" class="form-control" placeholder="ระบุอาการ" value="{{ (old('attitude.*')) }}" required oninvalid="this.setCustomValidity('กรุณากรอกข้อมูล')" oninput="this.setCustomValidity('')">
                                    @error('attitude.*')
                                        <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                                    @enderror
                                </td>
                                <td>
                                    <input type="date" name="dateOfHealth[]" class="form-control" value="{{ (old('dateOfHealth.*')) }}" required oninvalid="this.setCustomValidity('กรุณากรอกข้อมูล')" oninput="this.setCustomValidity('')">
                                    @error('dateOfHealth.*')
                                    <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                                    @enderror
                                </td>
                                <td>
                                    <input type="text" name="health_staff[]" class="form-control" placeholder="กรอกชื่อผู้รักษา" value="{{ (old('health_staff.*')) }}" required oninvalid="this.setCustomValidity('กรุณากรอกข้อมูล')" oninput="this.setCustomValidity('')">
                                    @error('health_staff.*')
                                        <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                                    @enderror
                                </td>
    ...
    <script>
        $('thead').on('click', '.addRow', function(){
                ...
                '<td>'+
                    ...
                    '@enderror'+
                '</td>'+
                '<td><input type="date" name="dateOfHealth[]" class="form-control" value="{{ (old("dateOfHealth.*")) }}" required oninvalid="this.setCustomValidity("กรุณากรอกข้อมูล")" oninput="this.setCustomValidity("")">'+
                    '@error("dateOfHealth.*")'+
                        '<div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>'+
                    '@enderror'+
                '</td>'+
                '<td><input type="text" name="health_staff[]" class="form-control" placeholder="กรอกชื่อผู้รักษา" value="{{ (old("health_staff.*")) }}" required oninvalid="this.setCustomValidity("กรุณากรอกข้อมูล")" oninput="this.setCustomValidity("")" >'+
                    '@error("health_staff.*")'+
                        '<div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>'+
                    '@enderror'+
                '</td>'+
