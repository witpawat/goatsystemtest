            ...
            <div class="row">
                ...
                <div class="form-group">
                    <table class="table">
                        <thead>
                            <tr>
                                <th>ชื่อ / รหัสแม่พันธุ์</th>
                                <th>ผสมพันธุ์ครั้งที่</th>
                                <th>วัน/เดือน/ปี</th>
                                <th>ชื่อ / รหัสพ่อพันธุ์</th>
                                <th>เจ้าหน้าที่</th>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>
                                    <input type="number" name="breedNo[]" class="form-control" placeholder="ผสมพันธุ์ครั้งที่" value="{{ (old('breedNo.*')) }}" min="1" required oninvalid="this.setCustomValidity('กรุณากรอกข้อมูล')" oninput="this.setCustomValidity('')">
                                    @error('breedNo.*')
                                        <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                                    @enderror
                                </td>
                                <td>
                                    <input type="date" name="dateOfBreed[]" class="form-control" value="{{ (old('dateOfBreed.*')) }}" required oninvalid="this.setCustomValidity('กรุณากรอกข้อมูล')" oninput="this.setCustomValidity('')">
                                    @error('dateOfBreed.*')
                                        <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                                    @enderror
                                </td>
                                <td>                                    
                                    <select name="father_breeder[]" class="form-control" id="father_breeder" required oninvalid="this.setCustomValidity('กรุณากรอกข้อมูล')" oninput="this.setCustomValidity('')">
                                        <option value="">--เลือก--</option>
                                ...
                                <td>
                                <input type="text" name="breed_staff[]" class="form-control" placeholder="กรอกชื่อเจ้าหน้าที่" value="{{ (old('breed_staff.*')) }}" required oninvalid="this.setCustomValidity('กรุณากรอกข้อมูล')" oninput="this.setCustomValidity('')">
                                    @error('breed_staff.*')
                                        <div class="alert alert-danger mt-1 mb-1">{{ $message }}</div>
                                    @enderror
                                </td>
